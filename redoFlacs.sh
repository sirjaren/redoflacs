...
#-----------
# 0.7.2 [12/04/11]
#         --Fixed a bug where finding FLAC files in the current
#           working directory yielded incorrect shell expansion.
#           Thanks to beermad for this fix!
# 0.7.1 [05/25/11]
#         --Fixed a bug with `eval` having issues with tags with
#           an odd number of double quotes (") (`eval`
#           is dangerous!)
# 0.7   [05/19/11]
#         --Added a warning when using the (--redo,-r) argument
#         --Added a (--disable-warning,-d) argument
#         --Cleaned up functions to allow safe exit upon
#           receiving SIGINT (Control-C)
#         --Updated documentation
# 0.6.1 [05/02/11]
#         --The directory where error logs are placed is now
#           configurable, defaulting to the HOME directory
#         --Updated documentation
# 0.6   [04/19/11]
#         --There is now multithreading available!
#           The default number of threads/cores used is 2
#         --Exported numerous functions to allow child processes
#           to use them when using more than one thread
#         --Rewrote most functions to allow multithreading
#         --Added a configuration option to change the number
#           of threads/cores to utilize upon runtime
#         --Added output to (--md5check,-m) argument
#         --Updated documentation
# 0.5   [04/17/11]
#         --The FLAC tags that can be saved are now configurable.
#           Under USER CONFIGURATION, set the tags you want kept
#         --Completely reimplemented the tagging functions to
#           allow for variable tagging
#         --Updated documentation
# 0.4.1 [04/08/11]
#         --Added a (--no-color,-n) argument
#         --Updated documentation
#         --Changed all STDOUT variables into functions
# 0.4   [04/08/11]
#         --Separated the clearing of the SEEKTABLE and
#           PADDING into a separate argument (--prune,-p)
#         --Redoing the FLAC tags is now a new argument
#           (--redo,-r)
#         --Added (--version,-v) arguments to check script
#           version
#         --Added output for the --prune and --redo arguments;
#           Useful for displaying progress and logging output
#         --Created a few more functions to display info to
#           STDOUT about when certain arguments are running
#         --Cosmetic changes to error reports
#         --Updated documentation on the new arguments
# 0.3   [04/05/11]
#         --Added example usage to long_help
#         --Separated much of the printing to STDOUT
#           into separate functions and variables
# 0.2   [04/04/11]
#         --Added a timestamp to each log
#           (prevents log concatenation)
#         --Reworked the help/documentation
# 0.1   [04/03/11]
#         --Initial Release
#----------------------------------------------------------------
...
# Must be exported in order for multithreading
function print_compressing_flac {
	printf "\r%75s${YELLOW}%s${NORMAL}%s\r%s${YELLOW}%s${NORMAL}%s" "[" "Compressing FLAC" "]" "     " "*" " $(basename "$i" | cut -c1-65)"
}
export -f print_compressing_flac
function print_testing_flac {
	printf "\r%75s${YELLOW}%s${NORMAL}%s\r%s${YELLOW}%s${NORMAL}%s" "[" "Testing FLAC" "]" "     " "*" " $(basename "$i" | cut -c1-65)"
}
...
function print_checking_md5 {
	printf "\r%75s${YELLOW}%s${NORMAL}%s\r%s${YELLOW}%s${NORMAL}%s" "[" "Checking MD5" "]" "     " "*" " $(basename "$i" | cut -c1-65)"
}
...
function print_analyzing_tags {
	printf "\r%75s${YELLOW}%s${NORMAL}%s\r%s${YELLOW}%s${NORMAL}%s" "[" "Analyzing Tags" "]" "     " "*" " $(basename "$i"     | cut -c1-65)"
}
export -f print_analyzing_tags 
function print_setting_tags {
	printf "\r%75s${YELLOW}%s${NORMAL}%s\r%s${YELLOW}%s${NORMAL}%s" "[" "Setting Tags" "]" "     " "*" " $(basename "$i"     | cut -c1-65)"
}
export -f print_setting_tags
function print_prune_flac {
	printf "\r%75s${YELLOW}%s${NORMAL}%s\r%s${YELLOW}%s${NORMAL}%s" "[" "Pruning Metadata" "]" "     " "*" " $(basename "$i"     | cut -c1-65)"
}
...
function long_help {
	cat << EOF
  Usage: $0 Usage: $0 [OPTION] [OPTION]... [PATH_TO_FLAC(s)]
  Options:
